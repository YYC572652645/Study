#include "OrderCastselect.h"
#include "ui_OrderCastselect.h"

OrderCastSelect::OrderCastSelect(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::OrderCastSelect)
{
    ui->setupUi(this);
...
OrderCastSelect::~OrderCastSelect()
{
    delete ui;
}
...
/********************查询学号*****************/
void OrderCastSelect::numerSelect(QString number)
{
    dataBaseCount=OrderCast->selectData(number,"","");
    if(dataBaseCount==0)
    {
    ...
    {
        ui->tableWidget->setItem(i,0,new QTableWidgetItem(OrderCast->OrderCastMessage[i].name));          //姓名
        ui->tableWidget->setItem(i,1,new QTableWidgetItem(OrderCast->OrderCastMessage[i].sex));           //性别
        ui->tableWidget->setItem(i,2,new QTableWidgetItem(OrderCast->OrderCastMessage[i].number));        //学号
        ui->tableWidget->setItem(i,3,new QTableWidgetItem(OrderCast->OrderCastMessage[i].profess));       //班级
        ui->tableWidget->setItem(i,4,new QTableWidgetItem(OrderCast->OrderCastMessage[i].OrderCastClass));  //班级排名
        ui->tableWidget->setItem(i,5,new QTableWidgetItem(OrderCast->OrderCastMessage[i].OrderCastPro));  //专业排名
        ui->tableWidget->setItem(i,6,new QTableWidgetItem(OrderCast->OrderCastMessage[i].OrderCastTime));    //年份
        ui->tableWidget->setItem(i,7,new QTableWidgetItem());  //学期
...
/********************查询姓名*****************/
void OrderCastSelect::nameSelect(QString name)
{
    dataBaseCount=OrderCast->selectData("",name,"");
    if(dataBaseCount==0)
    {
        ui->tableWidget->setRowCount(0);
        MessageDialog::getInstance()->setInfo(tr("系统提示"),tr("对不起，无数据！"),QPixmap(":/new/prefix1/image/icon-fail.png"),true);
...
/********************查询班级*****************/
void OrderCastSelect::classSelect(QString strClass)
{
    dataBaseCount=OrderCast->selectData("","",strClass);
    if(dataBaseCount==0)
    {
        ui->tableWidget->setRowCount(0);
        MessageDialog::getInstance()->setInfo(tr("系统提示"),tr("对不起，无数据！"),QPixmap(":/new/prefix1/image/icon-fail.png"),true);
...
/********************导出Excel*****************/
void OrderCastSelect::extendExcel(QStatusBar*statusBar,float width)
{
    ...
    cell->setProperty("Value","班级排名");         //写单元格值
    cell->clear();
    cell=workSheet->querySubObject("Cells(int,int)",1,6); //循环获取单元格中数据
    cell->setProperty("Value","专业排名");         //写单元格值
    cell->clear();
    ...
    for(int i=2;i<dataBaseCount+2;i++)
    {
        for(int j=1;j<8;j++)
        {
            ...
            switch(j)
            {
            case 1:cell->setProperty("Value",OrderCast->OrderCastMessage[dataCount].name);  break;       //写单元格值
            case 2:cell->setProperty("Value",OrderCast->OrderCastMessage[dataCount].sex);   break;       //写单元格值
            case 3:cell->setProperty("Value",OrderCast->OrderCastMessage[dataCount].number);break;       //写单元格值
            case 4:cell->setProperty("Value",OrderCast->OrderCastMessage[dataCount].profess);break;      //写单元格值
            case 5:cell->setProperty("Value",OrderCast->OrderCastMessage[dataCount].OrderCastClass);break; //写单元格值
            case 6:cell->setProperty("Value",OrderCast->OrderCastMessage[dataCount].OrderCastPro);break; //写单元格值
            case 7:cell->setProperty("Value",OrderCast->OrderCastMessage[dataCount].OrderCastTime);break;   //写单元格值
            }
        }
...
/********************导入Excel*****************/
void OrderCastSelect::importExcel(QStatusBar*statusBar,float width)
{
                ...
                for(int j=startColumn;j<startColumn+columnCount;j++)
                {
                    ...
                    if(j==startColumn)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                OrderCast->number=cellValue.toString();
                            }
                            else
                            {
                    ...
                    {
                        OrderCast->OrderCastClass=cellValue.toString();
                    }
                    else if(j==startColumn+2)
                    {
                        OrderCast->OrderCastPro=cellValue.toString();
                    }
                    else if(j==startColumn+3)
                    {
                        OrderCast->OrderCastTime=cellValue.toString();
                    }
                    else if(j==startColumn+4)
                    {
                        OrderCast->OrderCastTerm=cellValue.toString();
                    }

                    Count++;
                }
                OrderCast->insertData();
...
/********************删除数据*****************/
void OrderCastSelect::deleteData()
{
    if(OrderCast->number.isEmpty())
    {
        return;
    }
    if(OrderCast->deleteData())
    {
...
/********************获取当前选中行*****************/
void OrderCastSelect::selectText(const QModelIndex &index)
{
    rowCount=index.row();
    ...
    for(int i=0;i<8;i++)
    {
        ...
        switch(i)
        {
        case 2:OrderCast->number=item->text();break;
        case 4:OrderCast->OrderCastClass=item->text();break;
        case 5:OrderCast->OrderCastPro=item->text();break;
        case 6:OrderCast->OrderCastTime=item->text();break;
        case 7:OrderCast->OrderCastTerm=item->text();break;
        }
    }
...
/********************线程类构造函数*****************/
OrderCastThread::OrderCastThread(OrderCastSelect *widget, QStatusBar*statusBar, int width, int flage)
{
    this->widget=widget;
