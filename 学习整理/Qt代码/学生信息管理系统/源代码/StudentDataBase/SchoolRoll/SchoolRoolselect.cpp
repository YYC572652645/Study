#include "SchoolRoolselect.h"
#include "ui_SchoolRoolselect.h"

SchoolRoolSelect::SchoolRoolSelect(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::SchoolRoolselect)
{
    ui->setupUi(this);
    ...
    SchoolRool=new SchoolRoolDataBase();
...
SchoolRoolSelect::~SchoolRoolSelect()
{
    delete ui;
}
...
/********************查询学号*****************/
void SchoolRoolSelect::numerSelect(QString number)
{
    dataBaseCount=SchoolRool->selectData(number,"","");
    if(dataBaseCount==0)
    {
    ...
    {
        ui->tableWidget->setItem(i,0,new QTableWidgetItem(SchoolRool->SchoolRoolMessage[i].name));         //姓名
        ui->tableWidget->setItem(i,1,new QTableWidgetItem(SchoolRool->SchoolRoolMessage[i].sex));          //性别
        ui->tableWidget->setItem(i,2,new QTableWidgetItem(SchoolRool->SchoolRoolMessage[i].number));       //学号
        ui->tableWidget->setItem(i,3,new QTableWidgetItem(SchoolRool->SchoolRoolMessage[i].profess));      //专业班级
        ui->tableWidget->setItem(i,4,new QTableWidgetItem(SchoolRool->SchoolRoolMessage[i].SchoolRoolType)); //修复退种类
        ui->tableWidget->setItem(i,5,new QTableWidgetItem(SchoolRool->SchoolRoolMessage[i].SchoolRoolTime));  //时间
    }
...
/********************查询姓名*****************/
void SchoolRoolSelect::nameSelect(QString name)
{
    dataBaseCount=SchoolRool->selectData("",name,"");
    if(dataBaseCount==0)
    {
        ui->tableWidget->setRowCount(0);
        MessageDialog::getInstance()->setInfo(tr("系统提示"),tr("对不起，无数据！"),QPixmap(":/new/prefix1/image/icon-fail.png"),true);
...
/********************查询班级*****************/
void SchoolRoolSelect::classSelect(QString strClass)
{
    dataBaseCount=SchoolRool->selectData("","",strClass);
    if(dataBaseCount==0)
    {
        ui->tableWidget->setRowCount(0);
        MessageDialog::getInstance()->setInfo(tr("系统提示"),tr("对不起，无数据！"),QPixmap(":/new/prefix1/image/icon-fail.png"),true);
...
/********************导出Excel*****************/
void SchoolRoolSelect::extendExcel(QStatusBar*statusBar,float width)
{
    ...
    cell->setProperty("Value","修复退种类");         //写单元格值
    cell->clear();
    ...
    for(int i=2;i<dataBaseCount+2;i++)
    {
        for(int j=1;j<7;j++)
        {
            ...
            switch(j)
            {
            case 1:cell->setProperty("Value",SchoolRool->SchoolRoolMessage[dataCount].name);  break;    //写单元格值
            case 2:cell->setProperty("Value",SchoolRool->SchoolRoolMessage[dataCount].sex);   break;    //写单元格值
            case 3:cell->setProperty("Value",SchoolRool->SchoolRoolMessage[dataCount].number);break;    //写单元格值
            case 4:cell->setProperty("Value",SchoolRool->SchoolRoolMessage[dataCount].profess);break;   //写单元格值
            case 5:cell->setProperty("Value",SchoolRool->SchoolRoolMessage[dataCount].SchoolRoolType);break;  //写单元格值
            case 6:cell->setProperty("Value",SchoolRool->SchoolRoolMessage[dataCount].SchoolRoolTime);break;  //写单元格值
...
/********************导入Excel*****************/
void SchoolRoolSelect::importExcel(QStatusBar*statusBar,float width)
{
                ...
                for(int j=startColumn;j<startColumn+columnCount;j++)
                {
                    ...
                    if(j==startColumn)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                SchoolRool->number=cellValue.toString();
                            }
                            else
                            {
                    ...
                    {
                        SchoolRool->SchoolRoolType=cellValue.toString();
                    }
                    else if(j==startColumn+2)
                    {
                        SchoolRool->SchoolRoolTime=cellValue.toString();
                    }
                    Count++;
                }
                SchoolRool->insertData();
...
/********************删除数据*****************/
void SchoolRoolSelect::deleteData()
{
    if(SchoolRool->number.isEmpty())
    {
        return;
    }
    if(SchoolRool->deleteData())
    {
...
/********************获取当前选中行*****************/
void SchoolRoolSelect::selectText(const QModelIndex &index)
{
    rowCount=index.row();
    ...
    for(int i=0;i<7;i++)
    {
        ...
        switch(i)
        {
        case 2:SchoolRool->number=item->text();break;
        case 4:SchoolRool->SchoolRoolType=item->text();break;
        case 5:SchoolRool->SchoolRoolTime=item->text();break;
        }
    }
...
/********************线程类构造函数*****************/
SchoolRoolThread::SchoolRoolThread(SchoolRoolSelect *widget, QStatusBar*statusBar, int width, int flage)
{
    this->widget=widget;
