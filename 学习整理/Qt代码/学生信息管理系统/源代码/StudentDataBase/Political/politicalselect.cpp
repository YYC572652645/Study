#include "politicalselect.h"
#include "ui_politicalselect.h"

PoliticalSelect::PoliticalSelect(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::PoliticalSelect)
{
    ui->setupUi(this);
...
PoliticalSelect::~PoliticalSelect()
{
    delete ui;
}
...
/********************查询学号*****************/
void PoliticalSelect::numerSelect(QString number)
{
    dataBaseCount=poli->selectData(number,"","");
    if(dataBaseCount==0)
    {
    ...
    {
        ui->tableWidget->setItem(i,0,new QTableWidgetItem(poli->politicalMessage[i].name));         //姓名
        ui->tableWidget->setItem(i,1,new QTableWidgetItem(poli->politicalMessage[i].sex));          //性别
        ui->tableWidget->setItem(i,2,new QTableWidgetItem(poli->politicalMessage[i].number));       //学号
        ui->tableWidget->setItem(i,3,new QTableWidgetItem(poli->politicalMessage[i].profess));      //专业班级
        ui->tableWidget->setItem(i,4,new QTableWidgetItem(poli->politicalMessage[i].political));    //政治面貌
        ui->tableWidget->setItem(i,5,new QTableWidgetItem(poli->politicalMessage[i].groMemTime));   //入团时间
        ui->tableWidget->setItem(i,6,new QTableWidgetItem(poli->politicalMessage[i].parMemActTime));//入党积极分子时间
        ui->tableWidget->setItem(i,7,new QTableWidgetItem(poli->politicalMessage[i].parMebrTime));  //入党时间
    }
...
/********************查询姓名*****************/
void PoliticalSelect::nameSelect(QString name)
{
    dataBaseCount=poli->selectData("",name,"");
    if(dataBaseCount==0)
    {
        ui->tableWidget->setRowCount(0);
        MessageDialog::getInstance()->setInfo(tr("系统提示"),tr("对不起，无数据！"),QPixmap(":/new/prefix1/image/icon-fail.png"),true);
...
/********************查询班级*****************/
void PoliticalSelect::classSelect(QString strClass)
{
    dataBaseCount=poli->selectData("","",strClass);
    if(dataBaseCount==0)
    {
        ui->tableWidget->setRowCount(0);
        MessageDialog::getInstance()->setInfo(tr("系统提示"),tr("对不起，无数据！"),QPixmap(":/new/prefix1/image/icon-fail.png"),true);
...
/********************导出Excel*****************/
void PoliticalSelect::extendExcel(QStatusBar*statusBar,float width)
{
    ...
    for(int i=2;i<dataBaseCount+2;i++)
    {
        for(int j=1;j<9;j++)
        {
            ...
            switch(j)
            {
            case 1:cell->setProperty("Value",poli->politicalMessage[dataCount].name);  break;       //写单元格值
            case 2:cell->setProperty("Value",poli->politicalMessage[dataCount].sex);   break;       //写单元格值
            case 3:cell->setProperty("Value",poli->politicalMessage[dataCount].number);break;       //写单元格值
            case 4:cell->setProperty("Value",poli->politicalMessage[dataCount].profess);break;      //写单元格值
            case 5:cell->setProperty("Value",poli->politicalMessage[dataCount].political);break;    //写单元格值
            case 6:cell->setProperty("Value",poli->politicalMessage[dataCount].groMemTime);break;   //写单元格值
            case 7:cell->setProperty("Value",poli->politicalMessage[dataCount].parMemActTime);break;//写单元格值
            case 8:cell->setProperty("Value",poli->politicalMessage[dataCount].parMebrTime);break;  //写单元格值
            }
        }
...
/********************导入Excel*****************/
void PoliticalSelect::importExcel(QStatusBar*statusBar,float width)
{
                ...
                for(int j=startColumn;j<startColumn+columnCount;j++)
                {
                    ...
                    if(j==startColumn)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                poli->number=cellValue.toString();
                            }
                            else
                            {
                    ...
                    {
                        poli->groMemTime=cellValue.toString();
                    }
                    else if(j==startColumn+2)
                    {
                        poli->parMemActTime=cellValue.toString();
                    }
                    else if(j==startColumn+3)
                    {
                        poli->parMebrTime=cellValue.toString();
                    }
                    Count++;
                }
                poli->insertData();
...
/********************删除数据*****************/
void PoliticalSelect::deleteData()
{
    if(poli->number.isEmpty())
    {
        return;
    }
    if(poli->deleteData())
...
/********************获取当前选中行*****************/
void PoliticalSelect::selectText(const QModelIndex &index)
{
    rowCount=index.row();
    ...
    for(int i=0;i<8;i++)
    {
        ...
        switch(i)
        {
        case 2:poli->number=item->text();break;
        case 5:poli->groMemTime=item->text();break;
        case 6:poli->parMemActTime=item->text();break;
        case 7:poli->parMebrTime=item->text();break;
        }
    }
...
/********************线程类构造函数*****************/
PoliticalThread::PoliticalThread(PoliticalSelect *widget, QStatusBar*statusBar, int width, int flage)
{
    this->widget=widget;
