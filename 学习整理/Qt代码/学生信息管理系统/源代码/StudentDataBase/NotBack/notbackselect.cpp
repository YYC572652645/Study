#include "NotBackSelect.h"
#include "ui_NotBackSelect.h"

NotBackSelect::NotBackSelect(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::NotBackSelect)
{
    ui->setupUi(this);
...
/********************查询学号*****************/
void NotBackSelect::numerSelect(QString number)
{
    dataBaseCount=notBack->selectData(number,"","");
    if(dataBaseCount==0)
    {
    ...
    {
        ui->tableWidget->setItem(i,0,new QTableWidgetItem(notBack->NotBackMessage[i].name));          //姓名
        ui->tableWidget->setItem(i,1,new QTableWidgetItem(notBack->NotBackMessage[i].sex));           //性别
        ui->tableWidget->setItem(i,2,new QTableWidgetItem(notBack->NotBackMessage[i].number));        //学号
        ui->tableWidget->setItem(i,3,new QTableWidgetItem(notBack->NotBackMessage[i].profess));       //班级
        ui->tableWidget->setItem(i,4,new QTableWidgetItem(notBack->NotBackMessage[i].notBackTime));       //时间
...
/********************查询姓名*****************/
void NotBackSelect::nameSelect(QString name)
{
    dataBaseCount=notBack->selectData("",name,"");
    if(dataBaseCount==0)
    {
        ui->tableWidget->setRowCount(0);
        MessageDialog::getInstance()->setInfo(tr("系统提示"),tr("对不起，无数据！"),QPixmap(":/new/prefix1/image/icon-fail.png"),true);
...
/********************查询班级*****************/
void NotBackSelect::classSelect(QString strClass)
{
    dataBaseCount=notBack->selectData("","",strClass);
    if(dataBaseCount==0)
    {
        ui->tableWidget->setRowCount(0);
        MessageDialog::getInstance()->setInfo(tr("系统提示"),tr("对不起，无数据！"),QPixmap(":/new/prefix1/image/icon-fail.png"),true);
...
/********************导出Excel*****************/
void NotBackSelect::extendExcel(QStatusBar*statusBar,float width)
{
    ...
    for(int i=2;i<dataBaseCount+2;i++)
    {
        for(int j=1;j<6;j++)
        {
            ...
            switch(j)
            {
            case 1:cell->setProperty("Value",notBack->NotBackMessage[dataCount].name);  break;       //写单元格值
            case 2:cell->setProperty("Value",notBack->NotBackMessage[dataCount].sex);   break;       //写单元格值
            case 3:cell->setProperty("Value",notBack->NotBackMessage[dataCount].number);break;       //写单元格值
            case 4:cell->setProperty("Value",notBack->NotBackMessage[dataCount].profess);break;      //写单元格值
            case 5:cell->setProperty("Value",notBack->NotBackMessage[dataCount].notBackTime);break;      //写单元格值
            }
        }
...
/********************导入Excel*****************/
void NotBackSelect::importExcel(QStatusBar*statusBar,float width)
{
                ...
                for(int j=startColumn;j<startColumn+columnCount;j++)
                {
                    ...
                    if(j==startColumn)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                notBack->number=cellValue.toString();
                            }
                            else
                            {
                    ...
                    {
                        notBack->notBackTime=cellValue.toString();
                    }
                    Count++;
                }
                notBack->insertData();
...
/********************删除数据*****************/
void NotBackSelect::deleteData()
{
    if(notBack->number.isEmpty())
    {
        return;
    }
    if(notBack->deleteData())
    {
...
/********************获取当前选中行*****************/
void NotBackSelect::selectText(const QModelIndex &index)
{
    rowCount=index.row();
    ...
    for(int i=0;i<7;i++)
    {
        ...
        switch(i)
        {
        case 2:notBack->number=item->text();break;
        case 4:notBack->notBackTime=item->text();break;
        }
    }
...
/********************线程类构造函数*****************/
NotBackThread::NotBackThread(NotBackSelect *widget, QStatusBar*statusBar, int width, int flage)
{
    this->widget=widget;
