#include "cutschoolselect.h"
#include "ui_cutschoolselect.h"

CutSchoolSelect::CutSchoolSelect(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::CutSchoolSelect)
{
    ui->setupUi(this);
...
CutSchoolSelect::~CutSchoolSelect()
{
    delete ui;
}
...
/********************查询学号*****************/
void CutSchoolSelect::numerSelect(QString number)
{
    dataBaseCount=cutSchool->selectData(number,"","");
    if(dataBaseCount==0)
    {
    ...
    {
        ui->tableWidget->setItem(i,0,new QTableWidgetItem(cutSchool->cutSchoolMessage[i].name));          //姓名
        ui->tableWidget->setItem(i,1,new QTableWidgetItem(cutSchool->cutSchoolMessage[i].sex));           //性别
        ui->tableWidget->setItem(i,2,new QTableWidgetItem(cutSchool->cutSchoolMessage[i].number));        //学号
        ui->tableWidget->setItem(i,3,new QTableWidgetItem(cutSchool->cutSchoolMessage[i].profess));       //班级
        ui->tableWidget->setItem(i,4,new QTableWidgetItem(cutSchool->cutSchoolMessage[i].cutClass));      //旷课名称
        ui->tableWidget->setItem(i,5,new QTableWidgetItem(cutSchool->cutSchoolMessage[i].cutTime));       //旷课时间
...
/********************查询姓名*****************/
void CutSchoolSelect::nameSelect(QString name)
{
    dataBaseCount=cutSchool->selectData("",name,"");
    if(dataBaseCount==0)
    {
        ui->tableWidget->setRowCount(0);
        MessageDialog::getInstance()->setInfo(tr("系统提示"),tr("对不起，无数据！"),QPixmap(":/new/prefix1/image/icon-fail.png"),true);
...
/********************查询班级*****************/
void CutSchoolSelect::classSelect(QString strClass)
{
    dataBaseCount=cutSchool->selectData("","",strClass);
    if(dataBaseCount==0)
    {
        ui->tableWidget->setRowCount(0);
        MessageDialog::getInstance()->setInfo(tr("系统提示"),tr("对不起，无数据！"),QPixmap(":/new/prefix1/image/icon-fail.png"),true);
...
/********************导出Excel*****************/
void CutSchoolSelect::extendExcel(QStatusBar*statusBar,float width)
{
    ...
    cell->setProperty("Value","旷课名称");         //写单元格值
    cell->clear();
    cell=workSheet->querySubObject("Cells(int,int)",1,6); //循环获取单元格中数据
    cell->setProperty("Value","旷课时间");         //写单元格值
    ...
    for(int i=2;i<dataBaseCount+2;i++)
    {
        for(int j=1;j<7;j++)
        {
            ...
            switch(j)
            {
            case 1:cell->setProperty("Value",cutSchool->cutSchoolMessage[dataCount].name);  break;       //写单元格值
            case 2:cell->setProperty("Value",cutSchool->cutSchoolMessage[dataCount].sex);   break;       //写单元格值
            case 3:cell->setProperty("Value",cutSchool->cutSchoolMessage[dataCount].number);break;       //写单元格值
            case 4:cell->setProperty("Value",cutSchool->cutSchoolMessage[dataCount].profess);break;      //写单元格值
            case 5:cell->setProperty("Value",cutSchool->cutSchoolMessage[dataCount].cutClass);break;     //写单元格值
            case 6:cell->setProperty("Value",cutSchool->cutSchoolMessage[dataCount].cutTime);break;      //写单元格值
            }
        }
...
/********************导入Excel*****************/
void CutSchoolSelect::importExcel(QStatusBar*statusBar,float width)
{
                ...
                for(int j=startColumn;j<startColumn+columnCount;j++)
                {
                    ...
                    if(j==startColumn)
                    {
                        ...
                        else
                        {
                            ...
                            {
                                cutSchool->number=cellValue.toString();
                            }
                            else
                            {
                    ...
                    {
                        cutSchool->cutClass=cellValue.toString();
                    }
                    else if(j==startColumn+2)
                    {
                        cutSchool->cutTime=cellValue.toString();
                    }
                    Count++;
                }
                cutSchool->insertData();
...
/********************删除数据*****************/
void CutSchoolSelect::deleteData()
{
    if(cutSchool->number.isEmpty())
    {
        return;
    }
    if(cutSchool->deleteData())
    {
...
/********************获取当前选中行*****************/
void CutSchoolSelect::selectText(const QModelIndex &index)
{
    rowCount=index.row();
    ...
    for(int i=0;i<8;i++)
    {
        ...
        switch(i)
        {
        case 2:cutSchool->number=item->text();break;
        case 4:cutSchool->cutClass=item->text();break;
        case 5:cutSchool->cutTime=item->text();break;
        }
    }
...
/********************线程类构造函数*****************/
CutSchoolThread::CutSchoolThread(CutSchoolSelect *widget, QStatusBar*statusBar, int width, int flage)
{
    this->widget=widget;
